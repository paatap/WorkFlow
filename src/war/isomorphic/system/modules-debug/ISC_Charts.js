...
/*
  ...
  Version v11.0p_2016-05-23/EVAL Deployment (2016-05-23)
...
if (window.isc && isc.version != "v11.0p_2016-05-23/EVAL Deployment" && !isc.DevUtil) {
    isc.logWarn("SmartClient module version mismatch detected: This application is loading the core module from "
        + "SmartClient version '" + isc.version + "' and additional modules from 'v11.0p_2016-05-23/EVAL Deployment'. Mixing resources from different "
...
isc.evalBoundary;isc.B.push(isc.A._createExtraAxisLegendSwatch=function isc_FacetChart__createExtraAxisLegendSwatch(context,series,left,top,swatchSize){
...
isc.evalBoundary;isc.B.push(isc.A.formatAxisValue=function isc_FacetChart_formatAxisValue(value,forHorizontalAxis){
    if(!isc.isA.Number(value)){
        return value;
    }else{
...
isc.evalBoundary;isc.B.push(isc.A.drawRect=function isc_FacetChart_drawRect(left,top,width,height,props,context){
...
isc.evalBoundary;isc.B.push(isc.A._getNearestDrawnValue=function isc_FacetChart__getNearestDrawnValue(x,y,context,byDistanceOnly,keepText){
    if(context.chartType=="Scatter"||context.chartType=="Bubble"){
...
,isc.A._getZoomDataBounds=function isc_FacetChart__getZoomDataBounds(context,useAllData){
    context=context||this;
    if(context._zoomDataBounds!=null&&!useAllData){
        ...
        measure:function(){
            ...
            if(discrete){
                ...
                for(var i=start,triedRounding=false;i<end;){
                    ...
                    var label=this._getDateLabel(labelDate,scope,isFirstLabel);
                    pairs.push({dataValue:date,label:label});
                    if(scope==all){
                ...
                reserveLeft1=reserveRight1=(
                    vertical?0:Math.max(0,Math.ceil(maxHeight/2)-dataAndChartRectMargin));
                var startLabel=this._getUnscopedOrLogicalDateLabel(data[start].title);
                reserveLeft2=Math.max(
                    0,Math.ceil(chart.measureLabel(startLabel,props).width/2)-dataAndChartRectMargin);
                var endLabel=this._getUnscopedOrLogicalDateLabel(data[end-1].title);
                ...
                for(;i<end&&!(heightCutoffFlag&&widthCutoffFlag);++i){
                    var date=data[i].title,
                        label=this._getUnscopedOrLogicalDateLabel(date),
                        labelWidth=chart.measureLabel(label,props).width;
...
isc.evalBoundary;isc.B.push(isc.A._moveTo=function isc_c_FacetChart__moveTo(axes,locationA,locationB,bounds){
    var numAxes=axes.length,
